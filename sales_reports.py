...
def _rk_parts_series(frame: pd.DataFrame) -> pd.Series:
...
def _monthly_cumulative_value(row: pd.Series) -> float:
    """monthly 列：優先 qty_cumulative_raw（上傳累積），否則 qty。"""
    if "qty_cumulative_raw" in row.index:
        raw = row.get("qty_cumulative_raw")
        if raw is not None and pd.notna(raw):
            v = pd.to_numeric(raw, errors="coerce")
            if pd.notna(v):
                return float(v)
    v = pd.to_numeric(row.get("qty"), errors="coerce")
    return 0.0 if pd.isna(v) else float(v)


def reconcile_month_cumulative_display(
    df: pd.DataFrame,
    *,
    month_start: pd.Timestamp,
    month_end: pd.Timestamp,
    ...
    overlap = (sd <= me) & (rd >= ms)
    rk_all = _rk_parts_series(d)

    m_in_month = is_m & overlap & (rd >= ms) & (rd <= me)
    if not bool(m_in_month.any()):
        return df

    m_df = d.loc[m_in_month].copy()
    m_df["_rk"] = _rk_parts_series(m_df)
    drop_ix: set[object] = set()
    new_rows: list[dict] = []

    for rk in m_df["_rk"].unique():
        m_sub = d.loc[m_in_month & (rk_all == rk)].sort_values("report_date", kind="mergesort")
        if len(m_sub) == 0:
            continue
        w_ix = d.index[(~is_m) & (rk_all == rk) & overlap]
        parts = rk.split("\x1f")
        key_m = {WEEKLY_RECONCILE_KEYS[i]: parts[i] for i in range(len(WEEKLY_RECONCILE_KEYS))}

        drop_ix.update(m_sub.index.tolist())

        if len(w_ix) > 0:
            w_sub = d.loc[w_ix]
            sum_w = float(pd.to_numeric(w_sub["qty"], errors="coerce").fillna(0).sum())
            last_end = pd.to_datetime(w_sub["report_date"], errors="coerce").dt.normalize().max()
            m_last = m_sub.iloc[-1]
            ...
            prev_cum = 0.0
            for _, mrow in m_sub.iterrows():
                rd_i = pd.Timestamp(mrow["report_date"]).normalize()
                cum_i = _monthly_cumulative_value(mrow)
                seg_s = max(ms, prev_end + pd.Timedelta(days=1))
                seg_e = min(me, rd_i)
                if seg_s <= seg_e:
                    nr = {**tmpl, **key_m}
                    nr["Start_date"] = seg_s
    ...
    if not new_rows:
        return ensure_start_report_datetimes(kept)
...
def sales_df_for_calendar_month(
    ...
    *,
    ...
    - 列與 [month_start, month_end] 區間有交集者保留。
    - 再 reconcile_month_cumulative_display：累積 monthly 與週列依業務規則補尾段／展開為區間列。
    """
    if df is None or len(df) == 0:
        return df
    ms = pd.Timestamp(month_start).normalize()
    ...
    if len(out) == 0:
        return d.iloc[0:0]
    out = reconcile_month_cumulative_display(out, month_start=ms, month_end=me)
